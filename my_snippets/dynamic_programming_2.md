...
We want to find `dp[i][w] = max(dp[i-1,w], dp[i-1,w-wi]+ci, dp[i-1, w-2*wi]+2*ci..., dp[i-1,w-ui*wi]+ui*ci)`. Here `ci` = cost of joining components which is single road, hence `ci = 1`. Now let's divide the groups by remainder module `A[i]`. Then, for each group, the problem we need to solve is to find `min(a[i], a[i-1]+c, a[i-2]+2*c, ..., a[i-k]+k*c) = min(a[i], a[i-1]+1, a[i-2]+2, ..., a[i-k]+k)`, By setting `bi=ai-i*c=ai-i`, this expression is transformed into `min(bi+i,b[i-1]+(i-1)+1, ...)`, which is just `i+min(bi, b[i-1], ..., b[i-k])`
